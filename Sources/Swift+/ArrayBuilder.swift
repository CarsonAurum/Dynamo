...
/// 
/// This result builder uses generic methods that accept a variety of inputs to construct arrays in a declarative manner.
@resultBuilder
public enum ArrayBuilder {
    ...
    public static func buildPartialBlock<Element>(first: [Element]) -> [Element] { first }
    public static func buildPartialBlock<Element>(accumulated: [Element], next: Element) -> [Element] { accumulated + [next] }
    public static func buildPartialBlock<Element>(accumulated: [Element], next: [Element]) -> [Element] { accumulated + next }
