...
/// Runs an asynchronous operation by calling a closure that takes an `UnsafeContinuation<U, Never>`
/// and returns a `T` value. This can be used to wrap non-async APIs into async ones, allowing them to be
/// used with Swift's async/await syntax.
///
/// The closure `f` you provide to this function should take an `UnsafeContinuation` as a parameter and return a `T`.
/// The continuation is then resumed asynchronously, and the result of `f` and the resumed value are returned
/// as a tuple `(T, U)`.
///
...
/// - Returns: A tuple `(T, U)`, where `T` is the result of calling `f` and `U` is the result of the
/// continuation's resume.
/// - Note: Since this method works with an `UnsafeContinuation`, you need to ensure that the continuation is
...
/// ```
///
/// - Warning: This is an unsafe operation. If you forget to resume the continuation, or if you attempt to resume
/// it more than once, your program may crash or exhibit undefined behavior.
...
/// and may throw an `Error`. This can be used to wrap non-async APIs into async ones, allowing them to be used with
/// Swift's async/await syntax.
///
/// The closure `fn` you provide to this function should take an `UnsafeContinuation` as a parameter and return a `T`,
/// potentially throwing an error. The continuation is then resumed asynchronously, and the result of `fn` and the
/// resumed value are returned as a tuple `(T, U)`. If `fn` throws an error, it is caught and wrapped in
/// a `Result` type.
///
/// - Parameters:
///   - fn: A closure that accepts `UnsafeContinuation<U, Error>` as its parameter and may throw an `Error`,
///   returning a `T`.
/// - Returns: A tuple `(T, U)`, where `T` is the result of calling `fn` and `U` is the result of the continuation's
/// resume.
/// - Throws: An error if `fn` throws, or if the `Result` wrapping `fn`'s return value is a failure.
/// - Note: Since this method works with an `UnsafeContinuation`, you need to ensure that the continuation
/// is always resumed exactly once.
///
/// # Example
/// ```swift
/// do {
