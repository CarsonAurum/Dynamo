...
extension Publisher {
    ...
    /// After all the operations, it applies a logical AND operator (`&&`) to all values. If every operation in the Publisher
    /// chain has succeeded, it will return `true`.
    ///
    /// If any error occurs, this method catches the error and emits `false`, indicating that not all operations have succeeded.
    ///
    /// - Returns: A type-erased `Publosher` emitting a single Boolean value: `true` if all operations succeeded, `false` otherwise.
    ...
    /// After all the operations, it applies a logical AND operator (`&&`) to all values. If every operation in the Publisher
    /// chain has not failed, it will return `false`.
    ///
    /// If any error occurs, this method catches the error and emits `true`, indicating that an operation has failed.
    ///
    /// - Returns: A type-erased `Publisher` emitting a single Boolean value: `true` if any operation failed, `false` otherwise.
    ...
    /// Transforms a Publisher emitting `Result` values into a Publisher emitting the success or failure values directly.
    ///
    /// This method uses the `flatMap` operator to convert each `Result` in the original Publisher's output into a new Publisher.
    /// If the `Result` is a success, the new Publisher will emit the success value; if it's a failure, the new Publisher will emit an error.
    ///
    /// - Returns: A publisher which transforms the `Result` values emitted by the original Publisher into individual publishers
    /// that emit the success value or an error.
    ///
    ...
    ///
    /// - Note: The `Failure` type of the original Publisher must be `Never`, indicating that it doesn't emit any errors itself.
    ...
    ///
    /// This method applies the transformation function to each successful output of the Publisher chain, wrapping it in a `Result.success`.
    /// If any error occurs, the method catches the error, wraps it in a `Result.failure`, and applies the transformation function to it.
    ///
